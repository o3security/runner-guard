...
// KAYO runtime detections parsed from the agent's stdout, populated in cleanup().
let kayoDetections = [];

// Processes that belong to the GitHub Actions runner itself, not the user's
// build. Detections on these are the runner's own auth traffic (the internal
// API token it uses to talk to GitHub), not a real finding — filter them out.
const KAYO_RUNNER_NOISE = ["Runner.Listener", "Runner.Worker", "hosted-compute-", "provisioner"];

// Maps the agent's `[XxxDetected]` marker to a finding type + human title.
const KAYO_DETECTION_TYPES = {
  SecretDetected: { type: "secret_access", title: "Secret read by process" },
  FileAccess: { type: "sensitive_file_access", title: "Sensitive file access" },
  PrivEsc: { type: "privilege_escalation", title: "Privilege escalation attempt" },
  ReverseShell: { type: "reverse_shell", title: "Reverse shell" },
  ProcessExec: { type: "suspicious_exec", title: "Suspicious process execution" },
};

// parseKayoDetections scans the agent stdout for `[XxxDetected]\n{json}` blocks,
// parses each into a typed finding, drops runner-internal noise, and dedupes.
function parseKayoDetections(logText) {
  const out = [];
  const seen = new Set();
  const lines = logText.split("\n");
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/\[(\w*Detected|FileAccess|PrivEsc|ReverseShell|ProcessExec)\]/);
    if (!m) continue;
    // The JSON object follows the marker; collect until braces balance.
    let depth = 0, started = false, buf = "";
    for (let j = i + 1; j < lines.length && j < i + 60; j++) {
      const raw = lines[j].replace(/\x1b\[[0-9;]*m/g, ""); // strip ANSI
      for (const ch of raw) { if (ch === "{") { depth++; started = true; } else if (ch === "}") depth--; }
      buf += raw + "\n";
      if (started && depth <= 0) break;
    }
    let obj;
    try { obj = JSON.parse(buf.trim()); } catch { continue; }
    const comm = obj.comm || obj.process_info?.comm || "";
    const parent = obj.parent_comm || obj.process_info?.parent_comm || "";
    if (KAYO_RUNNER_NOISE.some(n => comm.startsWith(n) || parent.startsWith(n))) continue;
    const meta = KAYO_DETECTION_TYPES[m[1]] || { type: m[1].toLowerCase(), title: m[1] };
    const finding = {
      type: meta.type,
      title: obj.title || meta.title,
      severity: obj.severity || "medium",
      pattern_id: obj.pattern_id && obj.pattern_id !== "***" ? obj.pattern_id : "",
      matched_text: obj.matched_text || "",
      comm,
      parent_comm: parent,
      cmdline: obj.cmdline || obj.process_info?.cmdline || "",
      pid: obj.process_info?.pid || obj.pid || null,
      process_tree: obj.process_tree || obj.process_info?.process_tree || [],
    };
    const key = `${finding.type}|${finding.comm}|${finding.matched_text}|${finding.cmdline}`;
...
async function readLog(logType, logPath) {
  try {
    if (await fs.pathExists(logPath)) {
      ...
      if (content.trim()) {
        core.info(`--- Secure CI ${logType} ---`);
        core.info(content);
        core.info(`--- End Secure CI ${logType} ---`);
      }
      return content;
    }
  } catch (e) {
    core.warning(`Error reading Secure CI ${logType}: ${e.message}`);
  }
  return "";
}
...
`;
  ...
  // Runtime detections (KAYO) — same typed-table treatment as the network findings.
  let runtimeSection = "";
  if (kayoDetections.length > 0) {
    if (kayoDetections.some(d => d.severity === "critical" || d.severity === "high") && alertIcon === "✅") alertIcon = "🚨";
    const rows = kayoDetections.slice(0, 50).map(d => {
      const proc = d.comm ? `\`${d.comm}\`` : "–";
      const parent = d.parent_comm ? `\`${d.parent_comm}\`` : "–";
      const ev = d.matched_text || (d.cmdline ? `\`${d.cmdline.slice(0, 50)}\`` : "–");
      return `| \`${d.type}\` | ${d.severity} | ${d.title} | ${proc} | ${parent} | ${ev} |`;
    }).join("\n");
    runtimeSection = `
### Runtime Detections (${kayoDetections.length})

Kernel-level activity flagged by the runtime agent during this build.

| Type | Severity | Title | Process | Parent | Evidence |
|------|----------|-------|---------|--------|----------|
${rows}
`;
  }
  ...
  if (maliciousCount > 0) {
    alertIcon = "🚨";
    threatIntelSection = `
### 🛑 Malicious Egress Destinations (${maliciousCount})
...
`;
  ...
  const md = `
## ${alertIcon} O3 Secure CI — Security Summary
...
${baselineSection}
---
🛡️ Powered by [O3 Secure CI](https://github.com/o3security/secure-ci)
...
async function cleanup() {
  ...
  core.info("O3 Secure CI: stopping monitor and collecting results...");
  ...
  try {
    ...
    // 2. Signal Secure CI container (ecapture PID 1) to flush and exit.
    ...
    if (containerId) {
      core.info(`Sending SIGINT to Secure CI container ${containerId} (ecapture PID 1)...`);
    ...
    if (containerId) {
      core.info(`Stopping Secure CI container: ${containerId}`);
      try { await exec.exec("sudo", ["docker", "stop", "--timeout=10", containerId]); } catch (_) { }
    }
  } catch (e) {
    core.warning(`Error during Secure CI stop: ${e.message}`);
  }

  // ── KAYO runtime security teardown ─────────────────────────────────────
  // Mirrors the Secure CI stop sequence: SIGTERM → wait → docker stop. Tetragon's
  ...
  if (kayoContainerId) {
    ...
    if (kayoRunning) {
      ...
      try {
        // SIGTERM tells kayo to flush its detection report (attack-runtime.json)
        // and detach eBPF probes. Give it time to write before stopping.
    ...
    // Collect KAYO runtime detections into typed findings, the same shape the
    // network secret findings use, so they render in the Step Summary table.
    // The binary prints each detection to stdout as `[XxxDetected]` followed by
    // a JSON object; it also writes /app/attack-runtime.json on graceful exit.
    try {
      const kLogs = execSync(`sudo docker logs ${kayoContainerId} 2>&1`, {
        encoding: "utf8",
        maxBuffer: 16 * 1024 * 1024,
      });
      kayoDetections = parseKayoDetections(kLogs);
      core.info(`── KAYO: ${kayoDetections.length} runtime detection(s) after filtering runner-internal noise ──`);
      for (const d of kayoDetections.slice(0, 20)) {
        core.info(`  [${d.type}] ${d.severity} | ${d.title} | proc=${d.comm} | ${d.matched_text || ''}`);
      }
      // Keep the raw tail available for debugging.
      const tail = kLogs.split("\n").slice(-80).join("\n");
      core.info("── KAYO container logs (tail 80) ──");
  ...
  core.info("════════════════════════════════════════");
  core.info("RUNNER GUARD DIAGNOSTICS");
  core.info("════════════════════════════════════════");
  ...
  // chatty Secure CI container (ecapture can write tens of MB of TLS records).
  if (containerId) {
    try {
